...
document
  ...
  async function fetchEvents(page = 1) {
    try {
      const response = await fetch(`${api}?page=${page}&limit=${eventsPerPage}`);
      if (response.ok) {
        const data = await response.json();
        console.log(data); // Check the data structure in the console
        renderEvents(data); // Call the function to render the events
    ...
    events.forEach(event => {
      ...
      const row = `
        <tr>
          ...
          <td class="py-1">${dateObj.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</td>
  ...
  function renderPagination(totalCount) {
    ...
    for (let i = 1; i <= totalPages; i++) {
      paginationContainer.insertAdjacentHTML(
        "beforeend",
        `<a href="#" class="px-3 py-2 text-sm ${currentPage === i ? 'text-white bg-blue-600 border border-blue-600' : 'text-gray-500 bg-white border border-gray-200'} rounded hover:bg-gray-100" data-page="${i}">${i}</a>`
      );
    }
